...
export class CommandManager extends Disposable {
	...
	constructor(context: vscode.ExtensionContext, avatarManger: AvatarManager, dataSource: DataSource, extensionState: ExtensionState, repoManager: RepoManager, gitExecutable: GitExecutable | null, onDidChangeGitExecutable: Event<GitExecutable>, logger: Logger) {
		...
		this.registerCommand('an-dr-git.view', (arg) => this.view(arg));
		this.registerCommand('an-dr-git.addGitRepository', () => this.addGitRepository());
		this.registerCommand('an-dr-git.removeGitRepository', () => this.removeGitRepository());
		this.registerCommand('an-dr-git.clearAvatarCache', () => this.clearAvatarCache());
		this.registerCommand('an-dr-git.fetch', () => this.fetch());
		this.registerCommand('an-dr-git.endAllWorkspaceCodeReviews', () => this.endAllWorkspaceCodeReviews());
		this.registerCommand('an-dr-git.endSpecificWorkspaceCodeReview', () => this.endSpecificWorkspaceCodeReview());
		this.registerCommand('an-dr-git.resumeWorkspaceCodeReview', () => this.resumeWorkspaceCodeReview());
		this.registerCommand('an-dr-git.version', () => this.version());
		this.registerCommand('an-dr-git.openFile', (arg) => this.openFile(arg));
		...
		try {
			this.registerContext('an-dr-git:codiconsSupported', doesVersionMeetRequirement(vscode.version, VsCodeVersionRequirement.Codicons));
		} catch (_) {
			this.logger.logError('Unable to set Visual Studio Code Context "an-dr-git:codiconsSupported"');
		}
	}
	...
	/**
	 * The method run when the `an-dr-git.view` command is invoked.
	...
	/**
	 * The method run when the `an-dr-git.addGitRepository` command is invoked.
	 */
	...
	/**
	 * The method run when the `an-dr-git.removeGitRepository` command is invoked.
	 */
	...
	/**
	 * The method run when the `an-dr-git.clearAvatarCache` command is invoked.
	 */
	...
	/**
	 * The method run when the `an-dr-git.fetch` command is invoked.
	 */
	private fetch() {
		const repos = this.repoManager.getRepos();
	...
	/**
	 * The method run when the `an-dr-git.endAllWorkspaceCodeReviews` command is invoked.
	 */
	...
	/**
	 * The method run when the `an-dr-git.endSpecificWorkspaceCodeReview` command is invoked.
	 */
	...
	/**
	 * The method run when the `an-dr-git.resumeWorkspaceCodeReview` command is invoked.
	 */
	...
	/**
	 * The method run when the `an-dr-git.version` command is invoked.
	 */
	 ...
	 * The method run when the `an-dr-git.openFile` command is invoked.
