...
export class GitEditorManager extends Disposable {
	...
	public async showCommitMessageEditor(initialContent: string): Promise<string | null> {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'an-dr-commits-commit-message-'));
...
function getIPCHandlePath(nonce: string): string {
	if (process.platform === 'win32') {
		return '\\\\.\\pipe\\an-dr-commits-editor-' + nonce + '-sock';
	} else if (process.env['XDG_RUNTIME_DIR']) {
		return path.join(process.env['XDG_RUNTIME_DIR'] as string, 'an-dr-commits-editor-' + nonce + '.sock');
	} else {
		return path.join(os.tmpdir(), 'an-dr-commits-editor-' + nonce + '.sock');
	}
}
