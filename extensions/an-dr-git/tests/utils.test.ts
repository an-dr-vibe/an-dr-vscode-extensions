...
describe('openExtensionSettings', () => {
	it('Executes workbench.action.openSettings', async () => {
		...
		// Assert
		expect(vscode.commands.executeCommand).toHaveBeenCalledWith('workbench.action.openSettings', '@ext:an-dr.an-dr-git');
		expect(result).toBe(null);
	});
...
function expectedValueGitGraphUri(filePath: string, commit: string, repo: string, exists: boolean) {
	...
	return 'an-dr-git://file' + extension + '?' + Buffer.from(JSON.stringify({ filePath: filePath, commit: commit, repo: repo, exists: exists })).toString('base64');
}
