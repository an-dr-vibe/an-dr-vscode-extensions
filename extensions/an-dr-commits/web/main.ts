...
class CommitsView {
	...
	private getCommittedVisualHtml(author: string, email: string) {
		if (this.config.committedVisual === GG.CommittedVisualMode.Initials) {
			...
			const bg = getCommittedInitialsBackgroundColor(this.getAuthorAvatarSeed(author, email));
	...
	private getCommitContextMenuActions(target: DialogTarget & CommitTarget): ContextMenuActions {
		...
		return [[
		...
		], [
			{
				...
				onClick: () => {
					dialog.showConfirmation('Are you sure you want to permanently drop commit <b><i>' + abbrevCommit(hash) + '</i></b>?' + (this.onlyFollowFirstParent ? '<br/><i>Note: By enabling "Only follow the first parent of commits", some commits may have been hidden from the Commits View that could affect the outcome of performing this action.</i>' : ''), 'Yes, drop', () => {
		...
		document.body.addEventListener('contextmenu', (e: MouseEvent) => {
			...
			if (isExternalUrl || isInternalUrl) {
				...
				if (this.expandedCommit !== null && eventTarget.closest('#cdv') !== null) {
					...
					CommitsView.closeCdvContextMenuIfOpen(this.expandedCommit);
...
function bootstrap() {
	...
	const commits = new CommitsView(viewElem, VSCODE_API.getState() || null);
