    ...
    workspace: {
      ...
      getConfiguration: (section: string) => ({
        get: (key: string, def: unknown) => {
          if (section === "an-dr-com-mit-s" && key === "maxDepthOfRepoSearch") {
            return maxDepthVal;
          }
...
describe("watchForRepos", () => {
  ...
  describe("config change trigger", () => {
    it("does not call onReposFound when maxDepth did not increase", () => {
      ...
      mock.fireConfigChange("an-dr-com-mit-s.maxDepthOfRepoSearch");
    ...
    it("does not call onReposFound for an unrelated config key", () => {
      ...
      mock.fireConfigChange("an-dr-com-mit-s.graphStyle");
  ...
  describe("error commands (shown before any repo is found)", () => {
    it("an-dr-com-mit-s.view shows a modal error", async () => {
      watcher = watchForRepos(ctx, onReposFound, mockStatusBarItem);

      await mock.invokeCommand("an-dr-com-mit-s.view");
    ...
    it("an-dr-com-mit-s.clearAvatarCache shows a modal error", async () => {
      watcher = watchForRepos(ctx, onReposFound, mockStatusBarItem);

      await mock.invokeCommand("an-dr-com-mit-s.clearAvatarCache");
