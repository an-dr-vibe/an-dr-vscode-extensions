...
/**
 * Builds a filename/extension to inline-SVG map from the optional
 * `an-dr.an-dr-file-icons` extension. Returns an empty map when that
 * extension is not installed, so the file tree falls back to one generic
 * icon rather than failing.
 */
export function loadFileIcons(): Record<string, string> {
