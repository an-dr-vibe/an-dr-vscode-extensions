...
describe('AvatarManager', () => {
	...
	describe('fetchAvatarImage', () => {
		...
		describe('GitLab', () => {
			it('Should fetch a new avatar from GitLab (HTTPS Remote)', async () => {
				...
				expect(spyOnHttpsGet).toHaveBeenCalledWith({
					...
					headers: { 'User-Agent': 'vscode-an-dr-commits', 'Private-Token': 'w87U_3gAxWWaPtFgCcus' },
					agent: false,
					timeout: 15000
				}, expect.anything());
				expect(spyOnHttpsGet).toHaveBeenCalledWith({
		...
		it('Should fetch a new avatar, but not send it to the Commits View', async () => {
			// Setup
			spyOnGetRemoteUrl.mockResolvedValueOnce(null);
			mockHttpsResponse(200, 'binary-image-data');
			mockWriteFile(null);
		...
		it('Should fetch a new avatar, and log when it fails to send it to the Commits View', async () => {
			// Setup
			spyOnGetRemoteUrl.mockResolvedValueOnce(null);
			mockHttpsResponse(200, 'binary-image-data');
			mockWriteFile(null);
