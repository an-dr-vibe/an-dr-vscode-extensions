/**
 * Adversarial Unit Test Suite — rendererHelpers
 *
...
import {
    estW, clipToRect, levelCol,
    LEVEL_COLORS, EXTERNAL_COL,
} from '../../webview-src/graph-renderers/rendererHelpers';

// ══════════════════════════════════════════════════════════════════════════════
// PASS 1 — Functional coverage
// ══════════════════════════════════════════════════════════════════════════════

describe('Pass 1 – estW functional', () => {
    test('empty string returns base padding only',     () => expect(estW('')).toBe(28));
    test('single char',                               () => expect(estW('x')).toBe(35));
    test('3-char label scales linearly',              () => expect(estW('abc')).toBe(49));
    test('18-char label stays under cap',             () => expect(estW('a'.repeat(18))).toBe(154));
    test('long label caps at 160',                    () => expect(estW('a'.repeat(50))).toBe(160));
    test('19-char label already hits cap',            () => expect(estW('a'.repeat(19))).toBe(160));
});

describe('Pass 1 – clipToRect functional', () => {
    const r   = (v: number) => Math.round(v * 1e6) / 1e6;
    const clip = (sx: number, sy: number, tx: number, ty: number, hw: number, hh: number) => {
        const { x, y } = clipToRect(sx, sy, tx, ty, hw, hh);
        return { x: r(x), y: r(y) };
    };
...
describe('Pass 1 – levelCol functional', () => {
    test('level 0 → LEVEL_COLORS[0]', () => expect(levelCol(0, 'internal')).toEqual(LEVEL_COLORS[0]));
    test('level 1 → LEVEL_COLORS[1]', () => expect(levelCol(1, 'internal')).toEqual(LEVEL_COLORS[1]));
    test('level 5 → LEVEL_COLORS[5]', () => expect(levelCol(5, 'internal')).toEqual(LEVEL_COLORS[5]));
    test('level 6 clamps to last colour',  () => expect(levelCol(6,  'internal')).toEqual(LEVEL_COLORS[LEVEL_COLORS.length - 1]));
    test('level 99 clamps to last colour', () => expect(levelCol(99, 'internal')).toEqual(LEVEL_COLORS[LEVEL_COLORS.length - 1]));
    test('role=external → EXTERNAL_COL',   () => expect(levelCol(0, 'external')).toEqual(EXTERNAL_COL));
    test('role=folder → EXTERNAL_COL',     () => expect(levelCol(0, 'folder')).toEqual(EXTERNAL_COL));
    test('role=target → LEVEL_COLORS',     () => expect(levelCol(0, 'target')).toEqual(LEVEL_COLORS[0]));
});
...
describe('Pass 2 – clipToRect adversarial', () => {
    test('zero-width node + vertical motion → clips to vertical face (no NaN)', () => {
        ...
        const result = clipToRect(0, 0, 0, 100, 0, 14);
    ...
    test('source inside target bounding box returns target centre (fixed)', () => {
        ...
        const result = clipToRect(90, 0, 100, 0, 20, 14);
    ...
    test('source exactly on the boundary edge is NOT treated as inside', () => {
        ...
        const result = clipToRect(80, 0, 100, 0, 20, 14);
        expect(result.x).toBe(100);
        expect(result.y).toBe(0);
    });
    ...
    test('source just outside boundary clips normally', () => {
        ...
        const result = clipToRect(79, 0, 100, 0, 20, 14);
        expect(result.x).toBe(80);
        expect(result.y).toBe(0);
    });

    test('large coordinates stay finite', () => {
        const { x, y } = clipToRect(0, 0, 1e8, 1e8, 100, 28);
        expect(Number.isFinite(x)).toBe(true);
        expect(Number.isFinite(y)).toBe(true);
    });

    test('fractional positions stay finite', () => {
        const { x, y } = clipToRect(0.5, 0.3, 100.7, 80.2, 50, 14);
        expect(Number.isFinite(x)).toBe(true);
        expect(Number.isFinite(y)).toBe(true);
    });
    ...
    test('negative coordinates: clipped point lies on target boundary', () => {
        const hw = 30, hh = 14;
        const { x, y } = clipToRect(-200, -100, -50, -30, hw, hh);
        expect(Number.isFinite(x)).toBe(true);
        expect(Number.isFinite(y)).toBe(true);
...
describe('Pass 2 – levelCol adversarial', () => {
    test('negative level clamps to 0 (fixed)', () => {
        // Was: LEVEL_COLORS[-1] = undefined.  Fixed: Math.max(0, ...) clamps to 0.
        expect(levelCol(-1, 'internal')).toEqual(LEVEL_COLORS[0]);
    });

    test('NaN level clamps to last colour (fixed)', () => {
        // Was: LEVEL_COLORS[NaN] = undefined.  Fixed: !isFinite → last index.
        expect(levelCol(NaN, 'internal')).toEqual(LEVEL_COLORS[LEVEL_COLORS.length - 1]);
    });

    test('Infinity clamps to last colour', () => {
        expect(levelCol(Infinity, 'internal')).toEqual(LEVEL_COLORS[LEVEL_COLORS.length - 1]);
    });
    ...
    test('BUG: undefined role silently falls through to level branch', () => {
        ...
        expect(levelCol(0, undefined as any)).toEqual(LEVEL_COLORS[0]);
    });
});
...
describe('Pass 3 – scenario: deep graph node (6+ hops from root)', () => {
    ...
    // levelCol(99, role) must not return undefined.

    test('sentinel level 99 renders with a defined colour', () => {
        const col = levelCol(99, 'internal');
...
describe('Pass 3 – scenario: path-like node labels (file paths)', () => {
    ...
    test('27-char path label caps at 160', () => {
        ...
        expect(estW(label)).toBe(160);
    });

    test('short path label scales normally', () => {
        expect(estW('foo.ts')).toBe(6 * 7 + 28); // 70
    });
});
...
describe('Pass 3 – scenario: edge clipping for typical rose-layout distances', () => {
    ...
    // estW('SomeClass') = 9*7+28 = 91, hw=45.5, hh=14.

    test('horizontal edge: clipped endpoint is on left face of target', () => {
        const hw = estW('SomeClass') / 2;  // 45.5
        const hh = 14;
        const ep = clipToRect(0, 0, 200, 0, hw, hh);
        expect(ep.x).toBeCloseTo(200 - hw);
        expect(ep.y).toBe(0);
    });

    test('shallow diagonal edge: both endpoints on boundary and line runs source→target', () => {
        const hw = estW('Foo') / 2;   // 24.5
        const hh = 14;
        const ep  = clipToRect(0, 0,   300, 100, hw, hh);  // clips at target
        const sp2 = clipToRect(300, 100, 0, 0,   hw, hh);  // clips at source
...
describe('Pass 3 iter3 – clipToRect adx=0 guard (vertical motion)', () => {
    ...
    test('zero hw + zero hh + vertical → returns target center (degenerate point)', () => {
        ...
        const result = clipToRect(0, 0, 0, 100, 0, 0);
        expect(result.x).toBe(0);
