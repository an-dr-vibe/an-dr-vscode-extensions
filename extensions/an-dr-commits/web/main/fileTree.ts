...
function generateFileViewHtml(folder: FileTreeFolder, gitFiles: ReadonlyArray<GG.GitFileChange>, fileContextMenuOpen: number, type: GG.FileViewType, isUncommitted: boolean, selectedFilePath: string | null = null) {
	return type === GG.FileViewType.List
		? generateFileListHtml(folder, gitFiles, fileContextMenuOpen, isUncommitted, selectedFilePath)
		: generateFileTreeHtml(folder, gitFiles, fileContextMenuOpen, isUncommitted, selectedFilePath, true);
}

function generateFileTreeHtml(folder: FileTreeFolder, gitFiles: ReadonlyArray<GG.GitFileChange>, fileContextMenuOpen: number, isUncommitted: boolean, selectedFilePath: string | null, topLevelFolder: boolean): string {
	...
	const children = sortFolderKeys(curFolderInfo.folder).map((key) => {
		...
		return cur.type === 'folder'
			? generateFileTreeHtml(cur, gitFiles, fileContextMenuOpen, isUncommitted, selectedFilePath, false)
			: generateFileTreeLeafHtml(cur.name, cur, gitFiles, fileContextMenuOpen, isUncommitted, selectedFilePath);
	});
...
function generateFileListHtml(folder: FileTreeFolder, gitFiles: ReadonlyArray<GG.GitFileChange>, fileContextMenuOpen: number, isUncommitted: boolean, selectedFilePath: string | null) {
	...
	for (let i = 0; i < sortedLeaves.length; i++) {
		html += generateFileTreeLeafHtml(sortedLeaves[i].relPath, sortedLeaves[i].leaf, gitFiles, fileContextMenuOpen, isUncommitted, selectedFilePath);
	}
	return '<ul class="fileTreeFolderContents">' + html + '</ul>';
}

function generateFileTreeLeafHtml(name: string, leaf: FileTreeLeaf, gitFiles: ReadonlyArray<GG.GitFileChange>, fileContextMenuOpen: number, isUncommitted: boolean, selectedFilePath: string | null) {
	...
	if (leaf.type === 'file') {
		...
		const isSubmodule = fileTreeFile.submodule !== null;
		const diffPossible = isSubmodule || fileTreeFile.type === GG.GitFileStatus.Untracked || textFile;
		const changeTypeMessage = (isSubmodule ? 'Submodule • ' : '') + GIT_FILE_CHANGE_TYPES[fileTreeFile.type] + (fileTreeFile.type === GG.GitFileStatus.Renamed ? ' (' + escapeHtml(fileTreeFile.oldFilePath) + ' → ' + escapeHtml(fileTreeFile.newFilePath) + ')' : '');
		const selectedClass = fileTreeFile.newFilePath === selectedFilePath ? ' ' + CLASS_SELECTED : '';
		return '<li data-pathseg="' + encodedName + '"><span class="fileTreeFileRecord' + selectedClass + (leaf.index === fileContextMenuOpen ? ' ' + CLASS_CONTEXT_MENU_ACTIVE : '') + '" data-index="' + leaf.index + '"><span class="fileTreeFile' + (diffPossible ? ' gitDiffPossible' : '') + (leaf.reviewed ? '' : ' ' + CLASS_PENDING_REVIEW) + '" title="' + (diffPossible ? 'Click to View Diff' : 'Unable to View Diff' + (fileTreeFile.type !== GG.GitFileStatus.Deleted ? ' (this is a binary file)' : '')) + ' • ' + changeTypeMessage + '"><span class="fileTreeFileIcon">' + (isSubmodule ? ICONS.closedFolder : getFileIcon(name)) + '</span><span class="gitFileName ' + fileTreeFile.type + '">' + escapedName + '</span></span>' +
