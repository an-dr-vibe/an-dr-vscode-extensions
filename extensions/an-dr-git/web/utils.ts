/* Constants */
const VSCODE_API = (() => {
	const windowWithCachedApi = <Window & { __AN_DR_GIT_VSCODE_API__?: ReturnType<typeof acquireVsCodeApi> }>window;
	if (typeof windowWithCachedApi.__AN_DR_GIT_VSCODE_API__ === 'undefined') {
		windowWithCachedApi.__AN_DR_GIT_VSCODE_API__ = acquireVsCodeApi();
	}
	return windowWithCachedApi.__AN_DR_GIT_VSCODE_API__!;
})();

function debugWebviewLog(level: 'debug' | 'info' | 'warn' | 'error', message: string, details?: string) {
	try {
...
window.addEventListener('error', (event) => {
	...
	debugWebviewLog('error', 'window error event', detailParts.join('; '));
});
