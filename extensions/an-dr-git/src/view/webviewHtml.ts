...
export interface GitGraphWebviewHtmlRenderOptions {
	readonly panel: vscode.WebviewPanel;
	readonly nonce: string;
	readonly viewName: string;
	readonly gitExecutableUnknown: boolean;
	readonly initialState: GitGraphViewInitialState;
	readonly globalState: GitGraphViewGlobalState;
	readonly workspaceState: GitGraphViewWorkspaceState;
...
export function renderGitGraphWebviewHtml(options: GitGraphWebviewHtmlRenderOptions): GitGraphWebviewHtmlRenderResult {
	...
	for (let i = 0; i < options.initialState.config.graph.colours.length; i++) {
		colorVars += '--git-graph-color' + i + ':' + options.initialState.config.graph.colours[i] + '; ';
...
function getBodyHtml(options: GitGraphWebviewHtmlRenderOptions, numRepos: number): string {
