...
/**
 * Gets the life cycle state of Commits from the specified directory.
...
/**
 * Saves the life cycle state of Commits in the specified directory.
...
function sendEvent(event: LifeCycleEvent) {
	return new Promise<boolean>((resolve, reject) => {
		...
		const sendEvent: Omit<LifeCycleEvent, 'stage'> & { about: string, stage?: LifeCycleStage } = Object.assign({
			about: 'Information about this API is available at: https://api.mhutchie.com/vscode-an-dr-commits/about'
		}, event);
		...
		https.request({
			...
			path: '/vscode-an-dr-commits/' + (event.stage === LifeCycleStage.Install ? 'install' : event.stage === LifeCycleStage.Update ? 'update' : 'uninstall'),
			headers: {
