...
describe('RepoManager', () => {
	it('Should construct a RepoManager, and be disposed', async () => {
		...
		expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith('**/.vscode/vscode-an-dr-git.json');
	...
	describe('maxDepthOfRepoSearchChanged', () => {
		it('Should not trigger a workspace search if the value hasn\'t increased', async () => {
			...
			onDidChangeConfiguration.emit({
				affectsConfiguration: (section) => section === 'an-dr-git.maxDepthOfRepoSearch'
			});
		...
		it('Should not be triggered when other extension settings are changed', async () => {
			...
			// Run
			onDidChangeConfiguration.emit({
				affectsConfiguration: (section) => section === 'an-dr-git.date.type'
			});
	...
	describe('registerRepo', () => {
		it('Should register a new repository', async () => {
			...
			expect(utils.getPathFromStr(spyOnReadFile.mock.calls[0][0])).toStrictEqual('/path/to/workspace-folder2/repo/.vscode/vscode-an-dr-git.json');
	...
	describe('checkRepoForNewConfig', () => {
		...
		describe('validation', () => {
			const testValidationOfField = (fileKey: string, fileValue: any) => async () => {
				...
				// Assert
				expect(vscode.window.showErrorMessage).toHaveBeenCalledWith('The value for "' + fileKey + '" in the configuration file "/path/to/workspace-folder1/repo/.vscode/vscode-an-dr-git.json" is invalid.');
	...
	describe('onConfigWatcherCreateOrChange', () => {
		it('Should import the repository configuration when the file is created', async () => {
			...
			// Run
			emitOnDidCreate(vscode.Uri.file('/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-git.json'));
		...
		it('Should import the repository configuration when the file is changed', async () => {
			...
			// Run
			emitOnDidChange(vscode.Uri.file('/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-git.json'));
		...
		it('Shouldn\'t import the repository configuration when it is not for a known repository', async () => {
			...
			// Run
			emitOnDidChange(vscode.Uri.file('/path/to/workspace-folder1/repo2/.vscode/vscode-an-dr-git.json'));
	...
	describe('exportRepoConfig', () => {
		it('Should export the repository configuration', async () => {
			...
			expect(utils.getPathFromStr(spyOnWriteFile.mock.calls[0][0])).toBe('/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-git.json');
			expect(JSON.parse(spyOnWriteFile.mock.calls[0][1])).toStrictEqual({
				'exportedAt': 1587559258000
			});
			expect(vscode.window.showInformationMessage).toHaveBeenCalledWith('Successfully exported the Git Graph Repository Configuration to "/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-git.json".');
			expect(spyOnSaveRepos).toHaveBeenCalledWith({
		...
		it('Should return an error message when the file can\'t be written', async () => {
			...
			// Assert
			expect(result).toBe('Failed to write the Git Graph Repository Configuration File to "/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-git.json".');
			expect(spyOnSaveRepos).not.toHaveBeenCalled();
