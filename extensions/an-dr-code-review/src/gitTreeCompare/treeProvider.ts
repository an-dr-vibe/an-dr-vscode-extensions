...
import { TreeDataProvider, TreeItem, TreeItemCollapsibleState,
         ...
         workspace, commands, window, env, WorkspaceFoldersChangeEvent, TreeView, ThemeIcon, TreeItemCheckboxState, TreeCheckboxChangeEvent, Range, Selection, TextEditorRevealType } from 'vscode'
...
export class GitTreeCompareProvider implements TreeDataProvider<Element>, Disposable {
    ...
    private readConfig() {
        const config = workspace.getConfiguration(CONFIG_NAMESPACE);
    ...
    async getChildren(element?: Element): Promise<Element[]> {
        ...
        assert.ok(false, 'unsupported element type');
        return [];
    }
    ...
    *iterFiles(withinFolder: string | undefined = undefined) {
    ...
    viewAsTree(v: boolean) {
        ...
        commands.executeCommand('setContext', CONTEXT_NAMESPACE + '.viewAsList', viewAsList);
        this.log('Refreshing tree');
        this._onDidChangeTreeData.fire();
    }
    ...
    private updateFilterContext() {
        commands.executeCommand('setContext', CONTEXT_NAMESPACE + '.isFiltered', !!this.searchFilter);
    }
...
function toTreeItem(element: Element, openChangesOnSelect: boolean, iconsMinimal: boolean,
    ...
    if (element instanceof FileElement) {
        ...
        if (!element.isSubmodule) {
            ...
            item.command = {
                command: COMMAND_NAMESPACE + '.' + command,
                arguments: [element],
                title: ''
            };
        }
