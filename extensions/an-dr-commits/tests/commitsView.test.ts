...
describe('CommitsView', () => {
	...
	describe('WebviewPanel Construction', () => {
		...
		it('Should reapply required webview options when reviving a restored WebviewPanel', () => {
			// Setup
			const panel = vscode.window.createWebviewPanel('an-dr-commits', 'Commits', vscode.ViewColumn.One, {
				enableScripts: true,
				localResourceRoots: [vscode.Uri.file(path.join('/path/to/extension', 'media'))]
			});
	...
	describe('sendMessage', () => {
		...
		it('Should log an error message when Webview.postMessage rejects, and the CommitsView hasn\'t been disposed', async () => {
			...
			await waitForExpect(() => {
				...
				expect(spyOnLog).not.toHaveBeenCalled();
				expect(spyOnLogError).toHaveBeenCalledWith('Unable to send "viewScm" message to the Commits View.');
			});
		});
	...
	describe('getHtmlForWebview', () => {
		...
		it('Should get HTML when no Git executable is known', () => {
			...
			// Assert
			const mockedWebviewPanel = vscode.getMockedWebviewPanel(0);
			expect(mockedWebviewPanel.panel.webview.html).toContain('<h2>Unable to load Commits</h2>');
		...
		it('Should get HTML when no repositories are found in the workspace', () => {
			...
			const mockedWebviewPanel = vscode.getMockedWebviewPanel(0);
			expect(mockedWebviewPanel.panel.webview.html).toContain('<h2>Unable to load Commits</h2>');
			expect(mockedWebviewPanel.panel.webview.html).toContain('No Git repositories were found in the current workspace when it was last scanned by Commits.');
