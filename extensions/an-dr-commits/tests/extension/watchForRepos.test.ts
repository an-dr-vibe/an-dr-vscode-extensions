    ...
    workspace: {
      ...
      getConfiguration: (section: string) => ({
        get: (key: string, def: unknown) => {
          if (section === "an-dr-commits" && key === "maxDepthOfRepoSearch") {
            return maxDepthVal;
          }
...
describe("watchForRepos", () => {
  ...
  describe("config change trigger", () => {
    it("does not call onReposFound when maxDepth did not increase", () => {
      ...
      mock.fireConfigChange("an-dr-commits.maxDepthOfRepoSearch");
    ...
    it("does not call onReposFound for an unrelated config key", () => {
      ...
      mock.fireConfigChange("an-dr-commits.graphStyle");
  ...
  describe("error commands (shown before any repo is found)", () => {
    it("an-dr-commits.view shows a modal error", async () => {
      watcher = watchForRepos(ctx, onReposFound, mockStatusBarItem);

      await mock.invokeCommand("an-dr-commits.view");
    ...
    it("an-dr-commits.clearAvatarCache shows a modal error", async () => {
      watcher = watchForRepos(ctx, onReposFound, mockStatusBarItem);

      await mock.invokeCommand("an-dr-commits.clearAvatarCache");
