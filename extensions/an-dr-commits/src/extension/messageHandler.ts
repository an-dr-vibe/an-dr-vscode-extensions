...
function viewDiff(
  ...
  const comparison = createFileRevisionComparison(commitHash, oldFilePath, newFilePath, type);
  const abbrevHash = abbrevCommit(commitHash);
  const pathComponents = comparison.target.path.split("/");
  const title =
  ...
  return new Promise<boolean>((resolve) => {
    vscode.commands
      .executeCommand(
        "vscode.diff",
        encodeDiffDocUri(repo, comparison.base.path, comparison.base.commit),
        encodeDiffDocUri(repo, comparison.target.path, comparison.target.commit),
        title,
        { preview: true }
      )
      .then(() => resolve(true))
...
export function registerMessageHandlers(
  ...
  bridge.onMessage("selectRepo", (msg) => {
    ...
    repoManager.selectRepo(msg.repo);
  ...
  bridge.onMessage("sendCommitRangeToCodeReview", async (msg) => {
    bridge.post({
      command: "sendCommitRangeToCodeReview",
      available: await sendCommitRangeToCodeReview(msg.from, msg.to, msg.repository)
    });
  });
