...
export class RepoManager extends Disposable {
	...
	constructor(dataSource: DataSource, extensionState: ExtensionState, onDidChangeConfiguration: Event<vscode.ConfigurationChangeEvent>, logger: Logger) {
		...
		this.configWatcher = vscode.workspace.createFileSystemWatcher('**/.vscode/vscode-an-dr-git.json');
		...
		this.registerDisposables(
			...
			onDidChangeConfiguration((event) => {
				if (event.affectsConfiguration('an-dr-git.maxDepthOfRepoSearch')) {
	...
	/**
	 * Apply the new value of `an-dr-git.maxDepthOfRepoSearch` to the RepoManager.
	 */
		...
		try {
			...
			if (state && file !== null && typeof file.exportedAt === 'number' && file.exportedAt > state.lastImportAt) {
				...
				if (validationError === null) {
					...
					showErrorMessage('The value for "' + validationError + '" in the configuration file "' + getPathFromStr(path.join(repo, '.vscode', 'vscode-an-dr-git.json')) + '" is invalid.');
				}
			}
		} catch (_) { }
		return false;
...
function readExternalConfigFile(repo: string) {
	return new Promise<Readonly<ExternalRepoConfig.File> | null>((resolve) => {
		fs.readFile(path.join(repo, '.vscode', 'vscode-an-dr-git.json'), (err, data) => {
			if (err) {
				resolve(null);
			} else {
				try {
	...
	return new Promise<string>((resolve, reject) => {
		...
		fs.mkdir(vscodePath, (err) => {
			if (!err || err.code === 'EEXIST') {
				const configPath = path.join(vscodePath, 'vscode-an-dr-git.json');
