/**
 * Commits generates an event when it is installed, updated, or uninstalled, that is anonymous, non-personal, and cannot be correlated.
 * - Each event only contains the Commits and Visual Studio Code version numbers, and a 256 bit cryptographically strong pseudo-random nonce.
 ...
 *   using Visual Studio Code >= 1.41.0). These insights enable Commits to utilise the latest features of Visual Studio Code as soon as
 *   the majority of users are using a compatible version. The data cannot, and will not, be used for any other purpose.
 * - Full details are available at: https://api.mhutchie.com/vscode-an-dr-commits/about
 */
...
/**
 * Run on startup to detect if Commits has been installed or updated, and if so generate an event.
 * @param extensionContext The extension context of Commits.
 */
export async function onStartUp(extensionContext: vscode.ExtensionContext) {
	...
	if (state === null || state.current.extension !== versions.extension) {
		// This is the first startup after installing Commits, or Commits has been updated since the last startup.
 ...
 * and to a directory in this Commits installation (for use during future uninstalls).
 * @param extensionContext The extension context of Commits.
...
/**
 * Get a nonce generated for this installation of Commits.
