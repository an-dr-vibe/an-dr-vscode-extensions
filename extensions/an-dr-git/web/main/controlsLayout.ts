...
function gitGraphShowOverflowActions(view: any, event: MouseEvent) {
	handledEvent(event);
	const hiddenButtons = gitGraphGetTopBarButtons(view).filter((button) => button.visible && button.elem.classList.contains('overflowHidden'));
...
function gitGraphUpdateControlsLayout(view: any) {
	const buttons = gitGraphGetTopBarButtons(view);
	const isOverflowing = () => {
...
function gitGraphShowFindWidgetFromToggle(view: any) {
...
function gitGraphUpdateCompactFindWidgetState(view: any) {
	const compact = document.body.classList.contains('compactSearch');
	if (!compact) {
		view.compactFindWidgetPinnedOpen = false;
	...
	if (shouldShowWidget) {
		gitGraphPositionCompactFindWidget(view);
	}
}

function gitGraphPositionCompactFindWidget(view: any) {
	const host = document.getElementById('findWidgetHost');
	if (host === null) return;
