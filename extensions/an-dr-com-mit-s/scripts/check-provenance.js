/**
 * Provenance checker for the an-dr-commits -> an-dr-com-mit-s migration.
 *
 * Classifies code in an-dr-commits as either imported Git Graph baseline or
 * independently authored work, so authored code can be relicensed MIT and
 * carried over while baseline expression stays out of the MIT extension.
 *
...
const SCAN_EXTENSIONS = ['.ts', '.js', '.css', '.html'];
...
function buildBaselineIndex(baselineRev) {
	const files = runGit(['ls-tree', '-r', '--name-only', baselineRev])
		.split('\n')
		.map((f) => f.trim())
		.filter((f) => f && SCAN_EXTENSIONS.includes(path.extname(f)));
	...
	for (const file of files) {
		...
		for (let i = 0; i + SHINGLE <= lines.length; i++) {
			shingles.add(lines.slice(i, i + SHINGLE).map((l) => l.norm).join('\n'));
		}
	}
...
function listTargetFiles() {
	...
	return tracked
		.map((f) => f.trim())
		.filter((f) => {
			if (!f || !SCAN_EXTENSIONS.includes(path.extname(f))) return false;
			const rel = path.relative(TARGET_DIR, f).split(/[\\/]/);
			if (rel.some((seg) => SKIP_SEGMENTS.has(seg))) return false;
...
function classify(file, index) {
	...
	for (let i = 0; i + SHINGLE <= lines.length; i++) {
		...
		if (index.shingles.has(key)) {
			for (const l of window) copied.add(l.lineNo);
		}
	}
	...
	let verdict = 'REVIEW';
	if (ratio < YOURS_BELOW) verdict = 'YOURS';
	else if (ratio > BASELINE_ABOVE) verdict = 'BASELINE';
...
function main() {
	...
	if (annotateIdx !== -1) {
		...
		if (!target || target.startsWith('--')) {
			console.error(`--annotate requires a path relative to the repository root, for example ${TARGET_DIR}/src/inlineBlame.ts`);
			process.exit(2);
		}
		target = target.replace(/\\/g, '/');
		if (!fs.existsSync(path.join(repoRoot, target))) {
			console.error(`No such file: ${target}`);
			console.error(`Paths are relative to the repository root, for example ${TARGET_DIR}/src/inlineBlame.ts`);
			process.exit(2);
		}
	}
	...
	let yoursLines = 0;
	console.log(`Baseline: ${baselineRev}  (${index.fileCount} files, ${index.shingles.size} shingles)`);
	console.log(`Target:   ${TARGET_DIR}  (${results.length} files)\n`);
	console.log('BASELINE%  LINES  VERDICT   FILE');
	for (const r of results) {
		counts[r.verdict]++;
		if (r.verdict === 'YOURS') yoursLines += r.total;
