...
export class AvatarManager extends Disposable {
	...
	/**
	 * Creates the Commits Avatar Manager.
	...
	private fetchFromGithub(avatarRequest: AvatarRequestItem, owner: string, repo: string) {
		...
		https.get({
			...
			headers: { 'User-Agent': 'vscode-an-dr-commits' },
	...
	private fetchFromGitLab(avatarRequest: AvatarRequestItem) {
		...
		https.get({
			...
			headers: { 'User-Agent': 'vscode-an-dr-commits', 'Private-Token': 'w87U_3gAxWWaPtFgCcus' }, // Token only has read access
			agent: false, timeout: 15000
		}, (res) => {
			let respBody = '';
			res.on('data', (chunk: Buffer) => { respBody += chunk; });
	...
	private saveAvatar(email: string, image: string, identicon: boolean) {
		...
		this.emitAvatar(email).then(
			(sent) => this.logger.logDebug(sent
				? 'Sent Avatar for ' + maskEmail(email) + ' to the Commits View'
				: 'Avatar for ' + maskEmail(email) + ' is ready to be used the next time the Commits View is opened'
			),
			() => this.logger.logWarning('Failed to Send Avatar for ' + maskEmail(email) + ' to the Commits View')
		);
	}
}
