...
import { createRepositoryStatus } from "@/repo-manager/repositoryStatus";
...
export function createRepoManager(
  ...
  const repositoryStatus = createRepositoryStatus();
  ...
  function selectRepo(repo: string | null, isDirty = false) {
    repositoryStatus.update(repo, isDirty);
