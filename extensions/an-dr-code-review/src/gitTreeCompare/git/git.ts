...
function findGitWin32InPath(onValidate: (path: string) => boolean): Promise<IGit> {
	const whichPromise = new Promise<string>((c, e) => which('git.exe', (err: Error | null, resolvedPath?: string) => err ? e(err) : c(resolvedPath!)));
...
export class Git {
	...
	spawn(args: string[], options: SpawnOptions = {}): cp.ChildProcess {
		...
		if (options.cwd) {
			options.cwd = sanitizePath(options.cwd.toString());
		}
