...
All settings use the `an-dr-commits` prefix.
...
The extension is included by the repository's `install.ps1` installer. It is
linked as the application-scoped `an-dr-commits` extension in every VS Code
profile.

To work on it, build it directly:

```sh
# Node.js 20.19 or newer is required by the locked toolchain.
npm install
npm run compile
npm test
```

To run it in the editor, run `./install.ps1` from the repository root, then use
**an-dr: Commits: View Git Graph** from the Command Palette.
