...
const SCAN_ROOTS = new Set(["src", "web", "tests", "media"]);
...
function normalise(line) {
  ...
  if (/^(import|export)\b.*\bfrom\b/.test(text)) {
    return null;
  }
  if (/^(import|export)\s*[{*]/.test(text)) {
    return null;
  }
  if (/\brequire\s*\(/.test(text) && /^(const|let|var|import)\b/.test(text)) {
    return null;
  }
  return text;
}
...
function substantiveLines(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (norm !== null) {
      out.push({ lineNo: i + 1, norm, raw: lines[i] });
    }
  }
  return out;
}
