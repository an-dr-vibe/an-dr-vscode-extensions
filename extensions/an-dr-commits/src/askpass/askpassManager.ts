...
export class AskpassManager extends Disposable {
	...
	private onRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
		...
		req.on('end', () => {
			...
			vscode.window.showInputBox({ placeHolder: data.request, prompt: 'Commits: ' + data.host, password: /password/i.test(data.request), ignoreFocusOut: true }).then(result => {
				res.writeHead(200);
...
function getIPCHandlePath(nonce: string): string {
	if (process.platform === 'win32') {
		return '\\\\.\\pipe\\an-dr-commits-askpass-' + nonce + '-sock';
	} else if (process.env['XDG_RUNTIME_DIR']) {
		return path.join(process.env['XDG_RUNTIME_DIR'] as string, 'an-dr-commits-askpass-' + nonce + '.sock');
	} else {
		return path.join(os.tmpdir(), 'an-dr-commits-askpass-' + nonce + '.sock');
	}
}
