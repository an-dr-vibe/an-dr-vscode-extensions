...
import { shouldApplyGraphRefresh } from "./graphRefresh";
...
class GitGraphView {
  ...
  private appliedCommitGeneration: number = 0;
  private requestedCommitGeneration: number = 0;
  ...
  public loadCommits(
    ...
    if (!shouldApplyGraphRefresh(this.appliedCommitGeneration, generation)) {
      return;
    }
    this.appliedCommitGeneration = generation;
    if (
      !hard &&
  ...
  private requestLoadCommits(hard: boolean, loadedCallback: (changes: boolean) => void) {
    ...
    const generation = ++this.requestedCommitGeneration;
    sendMessage({
      command: "loadCommits",
  ...
  public showCommitDetails(commitDetails: GitCommitDetails, fileTree: GitFolder) {
    ...
    if (commitDetails.parents.length > 0) {
      html +=
        '<div id="sendCommitRangeToCodeReview" class="roundedBtn">' +
        l10n.sendToCodeReview +
        "</div>";
    }
    ...
    document.getElementById("sendCommitRangeToCodeReview")?.addEventListener("click", () => {
      const parent = commitDetails.parents[0];
      if (parent !== undefined) {
        sendMessage({
          command: "sendCommitRangeToCodeReview",
          repository: this.currentRepo!,
          from: parent,
          to: commitDetails.hash
        });
      }
    });
...
window.addEventListener("message", (event) => {
  ...
  switch (msg.command) {
    ...
    case "resetToCommit":
      ...
      break;
    case "sendCommitRangeToCodeReview":
      break;
    case "revertCommit":
