...
describe('RepoManager', () => {
	it('Should construct a RepoManager, and be disposed', async () => {
		...
		expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith('**/.vscode/vscode-an-dr-commits.json');
	...
	describe('registerRepo', () => {
		it('Should register a new repository', async () => {
			...
			expect(utils.getPathFromStr(spyOnReadFile.mock.calls[0][0])).toStrictEqual('/path/to/workspace-folder2/repo/.vscode/vscode-an-dr-commits.json');
	...
	describe('checkRepoForNewConfig', () => {
		...
		describe('validation', () => {
			const testValidationOfField = (fileKey: string, fileValue: any) => async () => {
				...
				// Assert
				expect(vscode.window.showErrorMessage).toHaveBeenCalledWith('The value for "' + fileKey + '" in the configuration file "/path/to/workspace-folder1/repo/.vscode/vscode-an-dr-commits.json" is invalid.');
	...
	describe('onConfigWatcherCreateOrChange', () => {
		it('Should import the repository configuration when the file is created', async () => {
			...
			// Run
			emitOnDidCreate(vscode.Uri.file('/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-commits.json'));

			// Assert
			await waitForExpect(() => expect(vscode.window.showInformationMessage).toHaveBeenCalledTimes(2));
			expect(vscode.window.showInformationMessage).toHaveBeenCalledWith('A newer Commits Repository Configuration File has been detected for the repository "repo1". Would you like to override your current repository configuration with the new changes?', 'Yes', 'No');
			expect(vscode.window.showInformationMessage).toHaveBeenCalledWith('Commits Repository Configuration was successfully imported for the repository "repo1".');
		...
		it('Should import the repository configuration when the file is changed', async () => {
			...
			// Run
			emitOnDidChange(vscode.Uri.file('/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-commits.json'));

			// Assert
			await waitForExpect(() => expect(vscode.window.showInformationMessage).toHaveBeenCalledTimes(2));
			expect(vscode.window.showInformationMessage).toHaveBeenCalledWith('A newer Commits Repository Configuration File has been detected for the repository "Old Name". Would you like to override your current repository configuration with the new changes?', 'Yes', 'No');
			expect(vscode.window.showInformationMessage).toHaveBeenCalledWith('Commits Repository Configuration was successfully imported for the repository "Name".');
			expect(spyOnBufferedQueueEnqueue).toHaveBeenCalledWith('/path/to/workspace-folder1/repo1');
		});
		...
		it('Shouldn\'t import the repository configuration when it is not for a known repository', async () => {
			...
			// Run
			emitOnDidChange(vscode.Uri.file('/path/to/workspace-folder1/repo2/.vscode/vscode-an-dr-commits.json'));
	...
	describe('exportRepoConfig', () => {
		it('Should export the repository configuration', async () => {
			...
			expect(utils.getPathFromStr(spyOnWriteFile.mock.calls[0][0])).toBe('/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-commits.json');
		...
		it('Should return an error message when .vscode can\'t be created', async () => {
			...
			// Assert
			expect(result).toBe('An unexpected error occurred while checking if the "/path/to/workspace-folder1/repo1/.vscode" directory exists. This directory is used to store the Commits Repository Configuration file.');
			expect(spyOnSaveRepos).not.toHaveBeenCalled();
		...
		it('Should return an error message when the file can\'t be written', async () => {
			...
			// Assert
			expect(result).toBe('Failed to write the Commits Repository Configuration File to "/path/to/workspace-folder1/repo1/.vscode/vscode-an-dr-commits.json".');
			expect(spyOnSaveRepos).not.toHaveBeenCalled();
