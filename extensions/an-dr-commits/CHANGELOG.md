...
* #471 Spaces can be automatically substituted with hyphens or underscores in reference inputs on dialogs (e.g. Create Branch, Add Tag, etc.), by configuring the new extension setting `an-dr-commits.dialog.general.referenceInputSpaceSubstitution`.
* #476 "Open File" action is now available in the Visual Studio Code Diff View Title Menu, when the Diff View is opened from the Commits View. (Requires Visual Studio Code >= 1.42.0)
* #479 New Repository Dropdown Order option "Workspace Full Path", that sorts repositories according to the Visual Studio Code Workspace Folder order, then alphabetically by the full path of the repository. This is the new default order for the `an-dr-commits.repositoryDropdownOrder` extension setting.
...
* #486 All Commits View Keyboard Shortcut extension settings can now alternatively be set to "UNASSIGNED", if you don't want to have a keybinding for a specific Keyboard Shortcut.
...
* #429 Stashes can be hidden on the Commits View by disabling the new extension setting `an-dr-commits.repository.showStashes`. It can be overridden per repository in the Commits View's Repository Settings Widget.
...
## 1.28.0 - 2020-12-01
* #399 Sign Commits and Tags created by actions in the Commits View, by enabling the new extension settings `an-dr-commits.repository.sign.commits` and `an-dr-commits.repository.sign.tags` respectively. This is an alternative to the existing method using Git config (`commit.gpgSign` and `tag.gpgSign`).
* #406 The keybindings for all Commits View keyboard shortcuts can now be configured using new extension settings `an-dr-commits.keyboardShortcut.*`.
* #407 Control whether Remote Head Symbolic References (e.g. origin/HEAD) are displayed in Commits via the new extension setting `an-dr-commits.repository.showRemoteHeads`. Thanks [Maciej Wilk (@wilkmaciej)](https://github.com/wilkmaciej) for helping with this!
* #409 Include built-in support for new emoji shortcodes recently added to [gitmoji](https://gitmoji.carloscuesta.me/).
* #412 New "Commits: Fetch from Remote(s)" command contributed to Visual Studio Code, that opens the Commits View and immediately runs "Fetch from Remote(s)".
* #414 Your Commits Repository Configuration can now be exported to a file that can be committed in the repository. It allows others working in the same repository to automatically use the same Commits configuration. To export your configuration, click the "Export Repository Configuration" button at the bottom of the Repository Settings Widget (on the Commits View).
* #416 The existing file encoding extension setting `an-dr-commits.fileEncoding` can now also be configured at a Workspace Folder Scope.
...
* #364 Parse and render a frequently used subset of inline Markdown formatting rules in commit messages and tag details (bold, italics, bold & italics, and inline code blocks). Markdown parsing can be disabled using the extension setting `an-dr-commits.markdown`.
...
* #389 Use the Visual Studio Code Color Theme's text selection background colour (set by some themes and users) in the Commits View.
...
* #393 New display option for the Uncommitted Changes in the graph. This is controlled by the new extension setting `an-dr-commits.graph.uncommittedChanges`.
...
* #398 New command "Commits: Get Version Information", that displays basic version information of the Commits installation, and allows it to be easily copied to the clipboard.
* Various code improvements.

## 1.26.0 - 2020-09-13
* #318 Customise the branches that are initially shown when the Commits View is first opened. Each repository can have it's initial branches configured from the Repository Settings Widget on the Commits View, or they may inherit the global configuration set by the extension settings `an-dr-commits.repository.onLoad.showCheckedOutBranch` & `an-dr-commits.repository.onLoad.showSpecificBranches`.
* #341 Added a new extension setting `an-dr-commits.repository.showRemoteBranches`, to set the default value of the "Show Remote Branches" repository setting.
* #356 More control over pruning when fetching:
    * Added a new extension setting `an-dr-commits.repository.fetchAndPruneTags`, to enable pruning tags when fetching from remote(s) using the Fetch button on the Commits View Control Bar. Default: false (disabled)
    * When fetching a specific remote via the Repository Settings Widget, a "Fetch Remote" dialog is now shown. This dialog provides new options for pruning remote-tracking references and tags. The default value of these options can be set using the new extension settings `an-dr-commits.dialog.fetchRemote.prune` & `an-dr-commits.dialog.fetchRemote.pruneTags`.
...
* #359 When the Commits View initially loads, Up / Down keys can now be used scroll the view.
* #361 Many extension settings have been renamed to leverage Visual Studio Code's setting grouping, to make Commits settings easier to find / discover. If you previously used any of the renamed settings, the value you previously set will still be used until you set the equivalent new setting.
...
## 1.25.0 - 2020-07-26
* #293 Added an "Open a Terminal for this Repository" button onto the Commits View Control Bar.
* #321 Added a new extension setting `an-dr-commits.repositoryDropdownOrder`, to set the order that repositories are sorted in the repository dropdown on the Commits View (only visible when more than one repository exists in the current Visual Studio Code Workspace). Default: Full Path
* #322 Added the "Merge into current branch..." action onto the remote branch context menu.
* #327 Added a new extension setting `an-dr-commits.dialog.cherryPick.noCommit`, to set the default value of the "No Commit" option on the Cherry Pick Dialog.
* #331 Added two extension settings to specify the message format of the squash commit optionally created via the Merge & Pull Branch Dialogs respectively. The supported formats are: the existing squash message generated by Commits, and the detailed squash message generated by Git (stored in .git/SQUASH_MSG). 
* #336 Added new extension settings to set the default options on the Pull Branch Dialog.
* #337 Enhanced the Commit Details View File Tree renderer to better display deeply nested directory structures, such that folders with a single child folder are compressed into a single combined folder element. This behaviour can be disabled by using the new extension setting `an-dr-commits.commitDetailsView.fileView.fileTree.compactFolders`.
* Significant code and UI improvements.

## 1.24.0 - 2020-06-14
* #271 Support for Visual Studio Code in Web Browsers. Commits officially supports Visual Studio Code in Microsoft's Visual Studio Codespaces in the Google Chrome browser, however the underlying changes allow Commits to be supported in most third-party Visual Studio Code browser-based platforms, and in most popular web browsers.
...
* #304 Override the globally configured commit ordering (configured by `an-dr-commits.repository.commits.order`) per repository from the Commits View Column Header context menu.
* #305 Upgraded the minimum version requirement of Visual Studio Code from 1.31.0 to 1.38.0. This change has been needed for some time, however I now have to make this change due to Visual Studio Code decreasing support for older Webview integrations for all extensions. Almost all users of Commits will be unaffected by this change. If you are one of the few Commits users who use a version of Visual Studio Code older than 1.38.0, please upgrade Visual Studio Code to continue receiving updates to Commits. 
* #316 Include built-in support for several new emoji shortcodes recently added to [gitmoji](https://gitmoji.carloscuesta.me/).
* #319 Added a new extension setting `an-dr-commits.enhancedAccessibility`, that enables visual file change A|M|D|R|U indicators in the Commit Details View for users with colour blindness. In the future, this setting will enable any additional accessibility related features of Commits that aren't enabled by default. Default: false (disabled)
* Various code improvements.

## 1.23.0 - 2020-05-24
* #89 Added a new "Include commits only mentioned by reflogs" option to the Commits View's Repository Settings Widget. The default value can be defined globally for all repositories using the new extension setting `an-dr-commits.repository.includeCommitsMentionedByReflogs`. Default: false (disabled)
* #201 New Keyboard Shortcuts in the Commits View for scrolling to stashes:
    * `CTRL/CMD + S`: Scrolls the Commits View to the first (or next) stash in the loaded commits.
    * `CTRL/CMD + SHIFT + S`: Scrolls the Commits View to the last (or previous) stash in the loaded commits.
* #277 Open the Visual Studio Code Diff View from the Commits View in a specific Editor Group. Previously `an-dr-commits.openNewTabEditorGroup` only supported `Active` & `Beside`, now it additionally supports `One`, `Two`, `Three`, ... , `Nine`.
...
* #283 After the Keyboard Shortcut `CTRL + H` is run in the Commits View to scroll to the HEAD commit, the commit now momentarily flashes to draw attention to it.
* #285 Added a new extension setting `an-dr-commits.repository.showUntrackedFiles`, that controls whether untracked files are included in the uncommitted changes on the Commits View. Default: true (enabled)
* #286 Added a "View File at this Revision" button for each file displayed in the Commit Details / Comparison Views.
* #287 Added a new "Only follow the first parent of commits" option to the Commits View's Repository Settings Widget. The default value can be defined globally for all repositories using the new extension setting `an-dr-commits.repository.onlyFollowFirstParent`. Default: false (disabled)
* #292 Create a *.tar or *.zip archive of the repository at any branch or tag from the corresponding context menu.
* #299 Previously when the Commit Details View was open on a commit, `Up` / `Down` keystrokes would open the Commit Details View on the commit directly above or below it on the Commits View. This is now augmented with `CTRL/CMD + Up` / `CTRL/CMD + Down` keystrokes, that open the Commit Details View on its child or parent commit on the same branch.
* #303 Added scroll bar support to dropdown menus on dialogs, to prevent dropdowns with many items overflowing off the bottom of the Commits View.
* Various code and UI improvements.

## 1.22.0 - 2020-03-21
* #231 New Command "Commits: Resume a specific Code Review in Workspace..." opens the Commits View to a Code Review that is already in progress.
* #248 Added a "No Commit" option to the "Merge Branch" & "Merge Commit" Dialogs. The default value of this new option can be set using the extension setting `an-dr-commits.dialog.merge.noCommit`. Default: false (disabled)
* #250 New "Pull Request Creation" Integration automates the opening and pre-filling of a Pull Request form, directly from a branches context menu.
    * Configured from the Repository Settings Widget.
        ...
        * Custom Pull Request providers can be configured using the extension setting `an-dr-commits.customPullRequestProviders` (e.g. for use with privately hosted Pull Request providers). Information on how to configure custom providers is available [here](https://github.com/mhutchie/vscode-an-dr-commits/wiki/Configuring-a-custom-Pull-Request-Provider).
...
* #251 Added a "Record Origin" option to the "Cherry Pick" Dialog. The default value of this new option can be set using the extension setting `an-dr-commits.dialog.cherryPick.recordOrigin`. Default: false (disabled)
* #253 New Command "Commits: End a specific Code Review in Workspace..." ends a specific Code Review without having to first open it in the Commits View.
...
* #240 Show the commit's signature status to the right of the Committer in the Commit Details View by enabling the new extension setting `an-dr-commits.repository.commits.showSignatureStatus`. Hovering over the signature icon displays a tooltip with the signature details.
* #241 When the view has been scrolled to the bottom, it will automatically load more commits if they exist (instead of having to press the "Load More Commits" button). The new extension setting `an-dr-commits.repository.commits.loadMoreAutomatically` can be used to disable this.
* #243 The parents of a commit in the Commit Details View can be clicked to open the Commit Details View for the parent (only if the parent is within the commits loaded in the Commits View).
* Significant code and UI improvements, including:
    * Overhaul of the Commits View data loading & refreshing mechanism (in preparation for upcoming features).
    * Improved the rendering performance and event handling of the Commits View.

## 1.20.0 - 2019-12-24
* #139 Added a new extension setting `an-dr-commits.repository.commits.mute.commitsThatAreNotAncestorsOfHead`, that when enabled will display commits that aren't ancestors of the checked-out branch / commit with a muted text colour. 
* #219 Added a new extension setting `an-dr-commits.repository.onLoad.scrollToHead`, that when enabled will scroll the Commits View to be centered on the commit referenced by HEAD when opening or switching repositories. Default: false (disabled)
* #222 Resolve symbolic links when Commits is opened via the Visual Studio Code Source Control View.
...
## 1.19.1 - 2019-11-27
* #221 Fixed a compatibility issue with Visual Studio Code Remote Development that was introduced in Commits 1.19.0.

## 1.19.0 - 2019-11-27
* #220 Issue Linking - Converts issue numbers in commit messages into hyperlinks, that open the issue in your issue tracking system. This is configured in the Repository Settings Widget (opened from the Commits View's Control Bar).
...
* #215 Added "Open Commits Extension Settings" button on the Repository Settings Widget.
* #216 Added a new "Show Tags" extension setting, that can be overridden per repository in the Commits View's Repository Settings Widget. Default: true (enabled)
* #218 New Keyboard Shortcut `CTRL/CMD + h` scrolls the Commits View to be centered on the commit referenced by HEAD.
...
## 1.18.0 - 2019-11-05
* #202 New File List View in the Commit Details / Comparison Views, as an alternative to the existing File Tree View. The default File View Type can be specified using the setting `an-dr-commits.commitDetailsView.fileView.type`. This can be overridden per repository using the new controls on the right side of the Commit Details / Comparison Views.
* #197 Improved the default column widths when the table is being automatically laid out on narrow width views.
* #198 Customise which context menu actions are visible with the new extension setting `an-dr-commits.contextMenuActionsVisibility`. For more information, see the documentation [here](https://github.com/mhutchie/vscode-an-dr-commits/wiki/Extension-Settings#context-menu-actions-visibility).
...
* #185 Added two new date format options, "ISO Date & Time" and "ISO Date Only", to the extension setting `an-dr-commits.dateFormat`.
* #186 Added a "Push to remote" option to the "Add Tag" Dialog, that enables the extension to push the tag to a remote once it is added. The default value of this new option can be set using the extension setting `an-dr-commits.dialog.addTag.pushToRemote`. Default: false (disabled)
...
* #48 Support for Stashes
    * Show Stashes on the Commits View.
    * Stash Context Menu:
        * Apply Stash...
...
* #172 Prevent Visual Studio Code automatically requesting package.json files from Commits in the background, when README.md file diffs are opened from the Commit Details & Comparison Views.
* #173 Use .gitmodules (if configured) to automatically add Git submodules to Commits.
...
* #178 New setting `an-dr-commits.repository.showCommitsOnlyReferencedByTags` specifies whether commits that are only referenced by tags should be shown in Commits. Default: true (enabled)
...
## 1.13.0 - 2019-08-16
* #143 New setting `an-dr-commits.repository.useMailmap` enables the use of .mailmap files when displaying author & committer names and email addresses.
...
* #146 New command "Commits: Remove Git Repository" removes a repository from Commits.
* #149 New setting `an-dr-commits.integratedTerminalShell` allows a specific Shell (not the default) to be used by the Visual Studio Code Integrated Terminal, when opened by Commits during Interactive Rebase's. For security reasons, this setting can only be specified in the User Settings, not in the Workspace Settings.
* #150 New setting `an-dr-commits.fileEncoding` specifies the character set encoding used when retrieving a specific version of repository files (e.g. in the Diff View). A list of all supported encodings can be found [here](https://github.com/ashtuchkin/iconv-lite/wiki/Supported-Encodings). Default: utf8
* #154 Fetch Enhancements:
    * New setting `an-dr-commits.repository.fetchAndPrune` enables pruning when fetching from remote(s) using the Fetch button on the Commits View Control Bar. Default: false (disabled)
    * Fetch or Prune a specific remote from the Repository Settings widget.
* #155 If a known sub repository is included in the Commit Details View file tree, clicking on it will now load it in the Commits View.
...
* #135 Commits now behaves the same as the Visual Studio Code Git Extensions git executable detection. This allows:
    * A better first experience for users who don't have a Git executable in their PATH.
    * Two new error pages (replacing the existing "Unable to load Commits" error page). These new pages provide more useful information and actions to users who are presented with an error.
...
* #117 New setting `an-dr-commits.repository.commits.order` allows you to choose the order of commits (date, date-author or topo). See [git log](https://git-scm.com/docs/git-log#_commit_ordering) for more information on each order option. Default: date
* #119 New option to immediately check out a branch after it has been created with the "Create Branch..." action.
* #123 Persist the "Show Remote Branches" checkbox state across Commits view sessions per repository.
...
* #100 Show merge commits with a muted text colour. This is controlled by the new setting `an-dr-commits.repository.commits.mute.mergeCommits`. Default: true (enabled). Thanks [Sebastian Lay (@sebastianlay)](https://github.com/sebastianlay)!
* #104 Common Emoji Shortcodes are automatically replaced with the corresponding emoji in commit messages (including all [gitmoji](https://gitmoji.carloscuesta.me/)). Custom Emoji Shortcode mappings can be defined in `an-dr-commits.customEmojiShortcodeMappings`.
...
## 1.8.0 - 2019-06-08
* #90 New command "Commits: Add Git Repository" in the Command Palette allows additional Git repositories to be added to Commits (e.g. sub-repos).
* #91 New setting to enable Commits to open to the repository containing the active Text Editor document `an-dr-commits.openToTheRepoOfTheActiveTextEditorDocument`. Default: false (disabled)
* #92 Various improvements for a better user experience:
    ...
    * Enabled by default the recently added setting `an-dr-commits.retainContextWhenHidden` to provide near-instant tab restoration when switching back to Commits.
...
* New ways to filter branches on the Commits view:
    * #77 Allow multiple branches to be selected in the 'Branches' dropdown.
    * #49 Predefine an array of custom glob patterns with the new setting `an-dr-commits.customBranchGlobPatterns`, allowing you to use them anytime from the 'Branches' dropdown. For example: `[{"name": "Feature Requests", "glob": "heads/feature/*"}]`
* #71 Choose from a variety of branch and tag label alignment options to better suit the projects you work on. The alignment options of the new setting `an-dr-commits.referenceLabels.alignment` are:
...
* #30 New setting `an-dr-commits.retainContextWhenHidden` enables faster Commits tab restoration at the cost of additional memory usage when Commits is opened, but not visible (running in the background). Default: false (not enabled). Thanks [Yu Zhang (@yzhang-gh)](https://github.com/yzhang-gh)!
* #76 Open and view a specific repo in Commits directly from the title of a Source Code Provider in the Visual Studio Code SCP View. The new setting `an-dr-commits.sourceCodeProviderIntegrationLocation` specifies if the Commits menu item is inline on the title of the Source Code Provider, or on the 'More actions...' menu. Default: Inline
* #78 Combine local and remote branch labels if they refer to the same branch, and are on the same commit. When combined, the local and remote branch context menus are available from the corresponding section of the label. The new setting `an-dr-commits.referenceLabels.combineLocalAndRemoteBranchLabels` allows you to disable this behaviour.
...
* #85 New setting `an-dr-commits.openNewTabEditorGroup` allows you to choose where you'd like the Visual Studio Code Diff to open, either in the Active pane, or Beside to active pane. Default: Active
...
* #73 New keyboard shortcuts:
    ...
    * CTRL/CMD + r: Refresh the Commits.
    * Enter: If a dialog is open, pressing enter submits the dialog, taking the primary (left) action.
* #74 Dock the Commit Details View to the bottom of the Commits view with the new setting `an-dr-commits.commitDetailsView.location`, instead of rendering inline with the graph. Default: Inline (with graph)

## 1.4.6 - 2019-04-30
* #33 Support for git repositories in subfolders. New setting `an-dr-commits.maxDepthOfRepoSearch` specifies the maximum depth of subfolders to search (default: 0).
...
* #59 Various performance improvements for: opening Commits, loading commits, and opening the commit details view.

## 1.4.5 - 2019-04-15
* #26 Fetch and show commit author / committer avatars from GitHub, GitLab & Gravatar. If you'd like to use this feature, you must enable the setting `an-dr-commits.repository.commits.fetchAvatars`. Thanks [Walter Meier (@meierw)](https://github.com/meierw) for helping with the development of this!
...
* #51 Observe Visual Studio Code theme changes while Commits is open, now required due to a change in Visual Studio Code 1.33.0.
...
* #41 Load the last viewed repo when opening Commits in a multi-root workspace.
* #42 New setting `an-dr-commits.date.type` to specify the date type to be displayed, either the author or commit date.

## 1.4.3 - 2019-03-17
* #17 Automatic refresh when repo changes while Commits is visible.
* #32 Checkout a specific commit from the commit context menu.
* #20 Hide the "Commits" status bar item when the workspace has no Git repository.
* #28 Fixed the text colour used for dropdowns and dialogs, to support use with other VSCode colour themes.
* Added the Commits icon to the tab when Commits is opened. By default the icon is coloured, but it can be set to greyscale with the new configuration setting `an-dr-commits.tabIconColourTheme`.

## 1.4.2 - 2019-03-10
* #22 New setting to show the current branch by default when Commits is opened, instead of showing all branches. By default `an-dr-commits.repository.onLoad.showCheckedOutBranch` is false.
...
* #7 Added a new extension setting `an-dr-commits.commitDetailsView.autoCenter` to enable / disable automatic centering of the commit details view.
...
* Press escape to close any open Commits dialog.
* #6: The command title in the Command Palette is changed to "Commits: View Commits (git log)".
...
## 1.1.0 - 2019-02-11
* Perform Git actions directly from Commits by right clicking on a commit / branch / tag:
...
## 1.0.0 - 2019-02-10
* Initial release
* Commits Visualisation
    * Select from Local & Remote Branches
    * Display Heads, Tags & Remotes
    * Configuration Settings:
        * `an-dr-commits.graph.colours` - Specifies the colours used on the graph.
        * `an-dr-commits.graph.style` - Specifies the style of the graph.
        * `an-dr-commits.date.format` - Specifies the number of commits to initially load.
        * `an-dr-commits.repository.commits.initialLoad` - Specifies the number of commits to initially load.
        * `an-dr-commits.repository.commits.loadMore` - Specifies the number of commits to load when the "Load More Commits" button is pressed (only shown when more commits are available).
        * `an-dr-commits.showStatusBarItem` - Show a Status Bar item which opens Commits when clicked.
        * `an-dr-commits.repository.showUncommittedChanges` - Show uncommitted changes (set to false to decrease load time on large repositories).
