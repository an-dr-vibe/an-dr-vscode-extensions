...
/**
 * Implements the Commits View's context menus.
 */
class ContextMenu {
	...
	/**
	 * Show a context menu in the Commits View.
	...
	public show(actions: ContextMenuActions, checked: boolean, target: ContextMenuTarget | null, event: MouseEvent, frameElem: HTMLElement, onClose: (() => void) | null = null, className: string | null = null) {
		...
		const x = event.clientX + menuBounds.width < vw
			? event.clientX - 2
			: Math.max(event.clientX - menuBounds.width + 2, 2);
		const y = event.clientY + menuBounds.height < vh
			? event.clientY - 2
			: Math.max(event.clientY - menuBounds.height + 2, 2);
	...
	/**
	 * Close the context menu (if one is currently open in the Commits View).
	 */
	public close() {
		if (this.elem !== null) {
			this.elem.remove();
	...
	/**
	 * Refresh the context menu (if one is currently open in the Commits View). If the context menu has a dynamic source,
	 * re-link it to the newly rendered HTML Element, or close it if the target is no longer visible in the Commits View.
	...
	/**
	 * Is a context menu currently open in the Commits View.
	...
	/**
	 * Is the target of the context menu dynamic (i.e. is it tied to a Git object & HTML Element in the Commits View).
