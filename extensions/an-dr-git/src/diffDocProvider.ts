...
export class DiffDocProvider extends Disposable implements vscode.TextDocumentContentProvider {
	public static scheme = 'an-dr-git';
	 ...
	 * @param uri The `an-dr-git://file.ext?encoded-data` URI.
...
/**
 * Represents the data passed through `an-dr-git://file.ext?encoded-data` URI's by the DiffDocProvider.
 */
type DiffDocUriData = {
	filePath: string;
	commit: string;
 ...
 * @returns A URI of the form `an-dr-git://file.ext?encoded-data` or `file://path/file.ext`
 */
...
/**
 * Decode the data from a `an-dr-git://file.ext?encoded-data` URI.
