# Commits extension for Visual Studio Code

View a Commits of your repository, and easily perform Git actions from the graph. Configurable to look the way you want!

![Recording of Commits](https://github.com/mhutchie/vscode-an-dr-commits/raw/master/resources/demo.gif)
...
* Commits View:
    ...
    * Filter the branches shown in Commits using the 'Branches' dropdown menu. The options for filtering the branches are:
        * Show All branches
        * Select one or more branches to be viewed
        * Select from a user predefined array of custom glob patterns (by setting `an-dr-commits.customBranchGlobPatterns`)
    ...
    * Repository Settings Widget:
        ...
        * Export your Commits Repository Configuration to a file that can be committed in the repository. It allows others working in the same repository to automatically use the same Commits configuration.
    * Keyboard Shortcuts (available in the Commits View):
        * `CTRL/CMD + F`: Open the Find Widget.
        * `CTRL/CMD + H`: Scrolls the Commits View to be centered on the commit referenced by HEAD.
        * `CTRL/CMD + R`: Refresh the Commits View.
        ...
        * When the Commit Details View is open on a commit:
            * `Up` / `Down`: The Commit Details View will be opened on the commit directly above or below it on the Commits View.
    ...
    * Common Emoji Shortcodes are automatically replaced with the corresponding emoji in commit messages (including all [gitmoji](https://gitmoji.carloscuesta.me/)). Custom Emoji Shortcode mappings can be defined in `an-dr-commits.customEmojiShortcodeMappings`.
...
* "Commits" launch button in the Status Bar
* "Commits: View Commits" launch command in the Command Palette
...
* **UI Density**: Choose **Big** (original spacing), **Normal** (the default balanced density), or **Compact** (maximum information density). The tab's controls keep their normal size; Activity Bar controls follow the selected density.
* **Commit Details View**:
    ...
    * **Location**: Specifies where the Commit Details View is rendered in the Commits View.
* **Context Menu Actions Visibility**: Customise which context menu actions are visible. For more information, see the documentation [here](https://github.com/mhutchie/vscode-an-dr-commits/wiki/Extension-Settings#context-menu-actions-visibility).
...
* **Custom Pull Request Providers**: An array of custom Pull Request providers that can be used in the "Pull Request Creation" Integration. For information on how to configure this setting, see the documentation [here](https://github.com/mhutchie/vscode-an-dr-commits/wiki/Configuring-a-custom-Pull-Request-Provider).
* **Date**:
    * **Format**: Specifies the date format to be used in the "Date" column on the Commits View.
    * **Type**: Specifies the date type to be displayed in the "Date" column on the Commits View, either the author or commit date.
...
* **Enhanced Accessibility**: Visual file change A|M|D|R|U indicators in the Commit Details View for users with colour blindness. In the future, this setting will enable any additional accessibility related features of Commits that aren't enabled by default.
...
* **Integrated Terminal Shell**: Specifies the path and filename of the Shell executable to be used by the Visual Studio Code Integrated Terminal, when it is opened by Commits.
* **Keyboard Shortcut > \***: Configures the keybindings used for all keyboard shortcuts in the Commits View.
...
* **Open New Tab Editor Group**: Specifies the Editor Group where Commits should open new tabs, when performing the following actions from the Commits View: Viewing the Visual Studio Code Diff View, Opening a File, Viewing a File at a Specific Revision.
* **Open to the Repo of the Active Text Editor Document**: Open the Commits View to the repository containing the active Text Editor document.
* **Reference Labels**:
...
* **Repository**:
    * **Commits**:
        ...
        * **Order**: Specifies the order of commits on the Commits View. See [git log](https://git-scm.com/docs/git-log#_commit_ordering) for more information on each order option.
    ...
    * **Fetch and Prune**: Before fetching from remote(s) using the Fetch button on the Commits View Control Bar, remove any remote-tracking references that no longer exist on the remote(s).
    * **Fetch And Prune Tags**: Before fetching from remote(s) using the Fetch button on the Commits View Control Bar, remove any local tags that no longer exist on the remote(s).
    * **Include Commits Mentioned By Reflogs**: Include commits only mentioned by reflogs in the Commits View (only applies when showing all branches).
    * **On Load**:
        * **Scroll To Head**: Automatically scroll the Commits View to be centered on the commit referenced by HEAD.
        * **Show Checked Out Branch**: Show the checked out branch when a repository is loaded in the Commits View.
        * **Show Specific Branches**: Show specific branches when a repository is loaded in the Commits View.
    * **Only Follow First Parent**: Only follow the first parent of commits when discovering the commits to load in the Commits View. See [--first-parent](https://git-scm.com/docs/git-log#Documentation/git-log.txt---first-parent) to find out more about this setting.
    * **Show Commits Only Referenced By Tags**: Show Commits that are only referenced by tags in Commits.
    * **Show Remote Branches**: Show Remote Branches in Commits by default.
    * **Show Remote Heads**: Show Remote HEAD Symbolic References in Commits.
    * **Show Stashes**: Show Stashes in Commits by default.
    * **Show Tags**: Show Tags in Commits by default.
    * **Show Uncommitted Changes**: Show uncommitted changes. If you work on large repositories, disabling this setting can reduce the load time of the Commits View.
    * **Show Untracked Files**: Show untracked files when viewing the uncommitted changes. If you work on large repositories, disabling this setting can reduce the load time of the Commits View.
    * **Sign**:
...
* **Tab Icon Colour Theme**: Specifies the colour theme of the icon displayed on the Commits tab.
...
* `an-dr-commits.view`: Commits: View Commits
* `an-dr-commits.addGitRepository`: Commits: Add Git Repository... _(used to add sub-repos to Commits)_
* `an-dr-commits.clearAvatarCache`: Commits: Clear Avatar Cache
* `an-dr-commits.endAllWorkspaceCodeReviews`: Commits: End All Code Reviews in Workspace
* `an-dr-commits.endSpecificWorkspaceCodeReview`: Commits: End a specific Code Review in Workspace... _(used to end a specific Code Review without having to first open it in the Commits View)_
* `an-dr-commits.fetch`: Commits: Fetch from Remote(s) _(used to open the Commits View and immediately run "Fetch from Remote(s)")_
* `an-dr-commits.removeGitRepository`: Commits: Remove Git Repository... _(used to remove repositories from Commits)_
* `an-dr-commits.resumeWorkspaceCodeReview`: Commits: Resume a specific Code Review in Workspace... _(used to open the Commits View to a Code Review that is already in progress)_
* `an-dr-commits.version`: Commits: Get Version Information
...
This extension is available on the [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=an-dr.an-dr-commits) for Visual Studio Code.
