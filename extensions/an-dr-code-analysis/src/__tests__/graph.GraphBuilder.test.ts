...
/** Shorthand: a→b call edge. */
function edge(from: vscode.CallHierarchyItem, to: vscode.CallHierarchyItem): CallEdge {
    return { from, to };
}
...
describe('buildCallGraph', () => {
    ...
    it('adds caller nodes with role=caller and an edge pointing to target', () => {
        ...
        const graph = buildCallGraph(target, [edge(caller, target)], 'callGraph', 2, 'clangd');
    ...
    it('adds callee nodes with role=callee and an edge from target', () => {
        ...
        const graph = buildCallGraph(target, [edge(target, callee)], 'callGraph', 2, 'clangd');
    ...
    it('deduplicates nodes when the same item appears as caller and callee', () => {
        ...
        const graph = buildCallGraph(target, [edge(shared, target), edge(target, shared)], 'callGraph', 2, 'clangd');
    ...
    it('includes both callers and callees together', () => {
        ...
        const graph = buildCallGraph(target, [edge(caller, target), edge(target, callee)], 'callGraph', 2, 'clangd');
...
describe('role when same item is both caller and callee', () => {
    it('node first seen as caller keeps role=caller even if also in outgoing; both edges present', () => {
        ...
        const graph = buildCallGraph(t, [edge(shared, t), edge(t, shared)], 'callGraph', 2, 'clangd');
...
describe('item with missing selectionRange and range', () => {
    ...
    it('two different functions at line 0 in different files produce unique ids', () => {
        ...
        const graph = buildCallGraph(a, [edge(b, a)], 'callGraph', 2, 'clangd');
    ...
    it('two callers with identical name+file+line produce one node and one edge (G2 fix)', () => {
        ...
        const graph = buildCallGraph(t, [edge(c1, t), edge(c2, t)], 'callGraph', 2, 'clangd');
...
describe('edge direction', () => {
    it('incoming call edge goes from caller to target', () => {
        ...
        const graph = buildCallGraph(t, [edge(caller, t)], 'callGraph', 2, 'clangd');
        expect(graph.edges[0].targetId).toBe(graph.targetId);
    ...
    it('outgoing call edge goes from target to callee', () => {
        ...
        const graph = buildCallGraph(t, [edge(t, callee)], 'callGraph', 2, 'clangd');
        expect(graph.edges[0].sourceId).toBe(graph.targetId);
        expect(graph.edges[0].targetId).not.toBe(graph.targetId);
    });
});

describe('multi-level edges', () => {
    it('depth-2 callee edge connects intermediate nodes, not target (foo→bar→baz)', () => {
        const foo = itemEx('foo', '/src/foo.c', 0);
        const bar = itemEx('bar', '/src/bar.c', 0);
        const baz = itemEx('baz', '/src/baz.c', 0);

        const graph = buildCallGraph(foo, [edge(foo, bar), edge(bar, baz)], 'callGraph', 2, 'clangd');
        ...
        expect(graph.edges.some(e => e.sourceId === fooNode.id && e.targetId === barNode.id)).toBe(true);
        expect(graph.edges.some(e => e.sourceId === barNode.id && e.targetId === bazNode.id)).toBe(true);
        expect(graph.edges.some(e => e.sourceId === fooNode.id && e.targetId === bazNode.id)).toBe(false);
    });

    it('depth-2 caller edge connects intermediate nodes, not target (qux→bar→foo)', () => {
        const foo = itemEx('foo', '/src/foo.c', 0);
        const bar = itemEx('bar', '/src/bar.c', 0);
        const qux = itemEx('qux', '/src/qux.c', 0);

        const graph = buildCallGraph(foo, [edge(bar, foo), edge(qux, bar)], 'callGraph', 2, 'clangd');

        const fooNode = graph.nodes.find(n => n.label === 'foo')!;
        const barNode = graph.nodes.find(n => n.label === 'bar')!;
        const quxNode = graph.nodes.find(n => n.label === 'qux')!;

        expect(graph.edges.some(e => e.sourceId === barNode.id && e.targetId === fooNode.id)).toBe(true);
        expect(graph.edges.some(e => e.sourceId === quxNode.id && e.targetId === barNode.id)).toBe(true);
        expect(graph.edges.some(e => e.sourceId === quxNode.id && e.targetId === fooNode.id)).toBe(false);
    });
});
