...
/**
 * Open the Visual Studio Code Settings Editor to the Commits Extension Settings.
 * @returns A promise resolving to the ErrorInfo of the executed command.
 */
export function openExtensionSettings(): Thenable<ErrorInfo> {
	return vscode.commands.executeCommand('workbench.action.openSettings', '@ext:an-dr.an-dr-commits').then(
		() => null,
		() => 'Visual Studio Code was unable to open the Commits Extension Settings.'
	);
}
 ...
 * @returns A promise resolving to the ErrorInfo of the executed command.
 */
export function viewDiff(repo: string, fromHash: string, toHash: string, oldFilePath: string, newFilePath: string, type: GitFileStatus, viewColumn: vscode.ViewColumn = getConfig().openNewTabEditorGroup) {
...
/**
 * Check whether Commits is running on a Windows-based platform.
...
/**
 * Find a Git executable that Commits can use.
...
/**
 * Find a Git executable on a Darwin-based platform that Commits can use.
 * @returns A Git executable.
 */
...
/**
 * Find a Git executable on a Windows-based platform that Commits can use.
 * @returns A Git executable.
 */
